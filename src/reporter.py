...
the reporting workflow modular, maintainable, and easy to extend.

The generated report includes general sales metrics, highest-performing
records, Top 5 product rankings, product, category, and monthly summaries,
...
def get_monthly_summary(analysis_result: Dict[str, Any]) -> str:
    """Generate the monthly sales summary section.

    Creates a display copy of the `monthly_summary` DataFrame and formats
    the `ingreso_total` column as currency before converting the data into
    a plain-text table without the pandas index.

    The resulting section displays the aggregated monthly sales information
    produced by the analysis module.

    Args:
        analysis_result: Dictionary containing the `monthly_summary`
            DataFrame produced by the sales analysis process.

    Returns:
        A formatted string containing the aggregated monthly sales summary.
    """
    monthly_summary_for_display = analysis_result["monthly_summary"].copy()
    monthly_summary_for_display["ingreso_total"] = monthly_summary_for_display["ingreso_total"].map(lambda x: f"${x:,.2f}")
    summary = []
    summary.append("RESUMEN POR MES")
    summary.append(f"\n{monthly_summary_for_display.to_string(index=False)}\n")
    return "\n".join(summary)
...
def generate_report(
    ...
    date. It then coordinates the report helper functions to generate the
    complete report.

    The report includes general sales metrics, highest-performing product and
    category records, Top 5 product rankings, product, category, and monthly
    summaries, validation errors, and validation warnings.
    ...
    Args:
        analysis_result: Dictionary containing the metrics, summaries,
            rankings, and highest-performing records produced by the sales
            analysis process.
        errors: List of dictionaries containing detailed validation errors.
        warnings: List of dictionaries containing non-critical validation
            warnings.
...
Generado el: {date.today()}""")
    ...
    summary.append(get_monthly_summary(analysis_result))
